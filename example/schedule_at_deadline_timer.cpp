...
// g++ -std=c++23 -Wall -O3 -Os -s -I include -l uring example/schedule_at_deadline_timer.cpp -o /tmp/schedule_at_deadline_timer
...
int main(int argc, char* argv[])
{
    ...
    auto time_point = net::deadline_timer::traits_type::now() + boost::posix_time::seconds(number);

    unifex::then(unifex::schedule_at(sch, time_point), []{ return 8; })
    | unifex::then([&](int n)
      {
          timed_out = true;
          std::cout << "schedule_at deadline_timer " << n << std::endl;
      })
