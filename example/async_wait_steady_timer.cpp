...
// g++ -std=c++23 -Wall -O3 -Os -s -I include -l uring example/async_wait_steady_timer.cpp -o /tmp/async_wait_steady_timer
...
int main(int argc, char* argv[])
{
    ...
    | unifex::upon_error([]<typename Error>(Error error)
      {
          if constexpr(std::is_same_v<Error, error_code_t>)
              std::cout << "async_wait steady_timer: " << error.message() << std::endl;
      })
    | snp::start_detached(); 
    ...
    std::cout << "async_wait steady_timer waited " << dur << " seconds" << std::endl;
